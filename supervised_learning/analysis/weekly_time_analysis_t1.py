...
def get_mean_difference(res_data, df_all, data_cat, model_name):
    ...
    if 'actual_SB' in list(df_all):
        ...
        sblpa_diff = (df_all['actual_SBLPA'] - df_all['predicted_SBLPA']) / 60
        sblpa_res = mean_confidence_interval(sblpa_diff, confidence=0.95)
        sblpa_text = "MVPA (MD: {} h, 95%CI: {} to {} h)".format(sblpa_res[0], sblpa_res[1], sblpa_res[2])
        ...
        res_data.append([data_cat, model_name, sb_text, lpa_text, sblpa_text, mvpa_text])
        ...
        res_data.append([data_cat, model_name, '', '', sblpa_text, mvpa_text])
...
if __name__ == '__main__':
    ...
    mean_diff_columns = ['Set', 'Model', 'SB', 'LPA', 'SBLPA', 'MVPA']
    mean_diff_data = []

    for data_cat in eval_category:

        class_3000_df = load_df(join(CNN_ROOT, 'regression', 'window-3000-overlap-1500', 'individual_results', data_cat))
        df_3000_summary = pd.merge(class_3000_df, participant_df, left_on='participant', right_on='Participant ID', how='left')

        class_6000_df = load_df(join(CNN_ROOT, 'regression', 'window-6000-overlap-3000', 'individual_results', data_cat))
        ...
        class_6000_df.to_csv('class_6000_df_{}.csv'.format(data_cat), index=None)
        class_hblr_df.to_csv('class_hblr_df_{}.csv'.format(data_cat), index=None)
        ...
        # mean_diff_data = get_mean_difference(mean_diff_data, df_hblr_summary, data_cat, 'HBLR')
        #
        result_data = []
    ...
    # Save mean diff data
    # pd.DataFrame(mean_diff_data, columns=mean_diff_columns).to_csv('Mean_DIFF_data_v3.csv', index=None)

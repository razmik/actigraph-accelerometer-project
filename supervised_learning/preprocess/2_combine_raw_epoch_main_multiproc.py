...
STAT_FEATURE_OUT_FOLDER = 'E:/Data/Accelerometer_Dataset_Rashmika/pre-processed/P2-Processed_Statistical_features-MULTI_TEST/'
...
def process_file(fname):
    ...
    input_details = pd.read_csv(fname, usecols=[0, 1, 2, 3, 4, 9, 10])
    ...
    for index, row in tqdm(input_details.iterrows(), total=input_details.shape[0]):
        ...
        if experiment == exclude_subject_1[0] and week == exclude_subject_1[1] and day == exclude_subject_1[
            2] and user == exclude_subject_1[3]:
            continue
...
if __name__ == "__main__":
    ...
    for id_filename in input_detail_filenames_list:

        process = multiprocessing.Process(target=process_file, args=(id_filename,))

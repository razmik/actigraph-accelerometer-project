...
def evaluate_models(data, status):
    ...
    data.loc[(0.26 < data['raw_wrist_sdvm']) & (data['raw_wrist_sdvm'] <= 0.79) & (
        data['raw_wrist_mangle'] <= -53), 'dt_estimated_intensity_category'] = 2
    data.loc[data['raw_wrist_sdvm'] > 0.79, 'dt_estimated_intensity_category'] = 2
...
input_file_path = ("D:/Accelerometer Data\Assessment\staudenmayer/"+experiment+"/"+week+"/"+day+"/Epoch60/").replace('\\', '/')
...
evaluate_models(data, 'Staudenmayer DT - 60 sec')
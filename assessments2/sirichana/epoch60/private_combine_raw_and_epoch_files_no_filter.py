...
def process_without_filter(starting_row, end_row, experiment, week, day, user, date, epoch, epoch_duration, device='Wrist'):

    wrist_raw_data_filename = "D:/Accelerometer Data/"+experiment+"/"+week+"/"+day+"/"+user+" "+device+" "+date+"RAW.csv".replace('\\', '/')
    epoch_filename = "D:\Accelerometer Data\ActilifeProcessedEpochs/"+epoch+"/"+experiment+"/"+week+"/"+day\
                     + "/processed_1min_ref/"+user+"_"+experiment+"_"+week.replace(' ', '_')+"_"+day+"_"\
                     + date + "60s.csv".replace('\\', '/')

    path_components = wrist_raw_data_filename.split('/')

    output_path = "D:/Accelerometer Data/Assessment/sirichana/"
    ...
    """
    raw_data_wrist['svm'] = np.sqrt([(raw_data_wrist.X ** 2) + (raw_data_wrist.Y ** 2) + (raw_data_wrist.Z ** 2)])[0] - 1

    wrist_grouped_temp = raw_data_wrist.groupby(raw_data_wrist.index // n)
    aggregated_wrist = pd.DataFrame()
    aggregated_wrist['svm'] = wrist_grouped_temp['svm'].sum() * 0.4

    """
    Include the epoch counts for 60 seconds and CPM values in aggregated dataframe
    """

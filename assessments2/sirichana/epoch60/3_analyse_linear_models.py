...
def evaluate_models(data, status='Sirichana - 60 epochs'):
    ...
    data.loc[data['actilife_waist_intensity'] < 1.5, 'target_met_category_freedson_intensity'] = 1
    data.loc[(1.5 <= data['actilife_waist_intensity']) & (data['actilife_waist_intensity'] < 3), 'target_met_category_freedson_intensity'] = 2
    data.loc[3 <= data['actilife_waist_intensity'], 'target_met_category_freedson_intensity'] = 3
...
if __name__ == '__main__':
    ...
    prediction_path = "D:/Accelerometer Data/Processed/" + experiment + "/" + week + "/" + day1 + "/filtered\Epoch60/".replace(
        '\\', '/')

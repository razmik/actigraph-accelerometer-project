...
def process_with_filter(starting_row, end_row, experiment, week, day, user, date, device='Wrist'):
    ...
    epoch_filename = ("D:\Accelerometer Data\ActilifeProcessedEpochs/Epoch60/" + experiment + "/" + week + "/" + day
                      + "/processed/" + user + "_" + experiment + "_" + week.replace(' ', '_') + "_" + day + "_" + date + "60sec.csv.csv").replace('\\', '/')
    ...
    epoch_data = pd.read_csv(epoch_filename, skiprows=epoch_start, nrows=len(aggregated_wrist), usecols=[6, 7])
    epoch_data.columns = ['actilife_waist_ee', 'actilife_waist_intensity']

...
epoch_folder = 'D:\Accelerometer Data\Processed\LSM2\Week 1\Wednesday/not_filtered\epoch_5\montoye_2015/'.replace('\\', '/')
predictions_folder = 'D:\Accelerometer Data\Montoye\Features\LSM2\Week 1\Wednesday\Montoye_2015_predictions/5sec/result_files/'.replace('\\', '/')
output_folder = 'D:\Accelerometer Data\Montoye\Features\LSM2\Week 1\Wednesday\Montoye_2015_predictions/5sec/combined/'.replace('\\', '/')
...
def met_to_intensity(row):
    intensity = result_data['actilife_waist_intensity'][row.name]
...
if len(epoch_data_files) == len(prediction_data_files):
    for epoch_file, pred_file in zip(epoch_data_files, prediction_data_files):
        ...
        result_data['montoye_2015_hip_predicted_intensity_by_ee'] = result_data.apply(met_to_intensity, axis=1)

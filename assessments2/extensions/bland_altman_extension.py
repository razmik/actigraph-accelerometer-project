...
class BlandAltman:
    ...
    @staticmethod
    def bland_altman_paired_plot_tested(dataframe, plot_title, plot_number, log_transformed=False, min_count_regularise=False, prediction_column='predicted_ee_transformed'):
        ...
        if log_transformed:
            ...
            dataframe = dataframe.assign(predicted_ee_transformed=np.log10(dataframe[prediction_column]))

        dataframe = dataframe.assign(mean=np.mean([dataframe.as_matrix(columns=['waist_ee_transformed']),
                                     dataframe.as_matrix(columns=[prediction_column])], axis=0))
        dataframe = dataframe.assign(diff=dataframe['waist_ee_transformed'] - dataframe[prediction_column])
    ...
    def clean_data_points(data):
        ...
        data.loc[(data['predicted_ee'] <= 1), 'predicted_ee_transformed'] = 1

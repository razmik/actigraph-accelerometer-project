...
root_folder = 'E:/Data/Accelerometer_Montoye_ANN/'.replace('\\', '/')
                    ...
                    for wrist in wrists:
                        for model in models:
                            ...
                            for key, value in time_epoch_dictionary.items():
                                ...
                                incorrect_filename = root_folder + '/' + year + '/' + wrist + '/' + key + '/' + \
                                                     model + '/combined/' + user + '_' + date + '_' + 'row_' + \
                                                     str(int(incorrect_starting_row / value)) + '_to_' + \
                                                     str(int(incorrect_end_row / value)) + '_' + wrist + '_combined.csv'

                                corrected_file_start = int(starting_row / value)
                                corrected_file_end = int(end_row / value)

                                correct_filename = root_folder + '/' + year + '/' + wrist + '/' + key + '/' + \
                                                     model + '/combined/' + user + '_' + date + '_' + 'row_' + \
                                                     str(int(corrected_file_start / value)) + '_to_' + \
                                                     str(int(corrected_file_end / value)) + '_' + wrist + '_combined.csv'

...
non_filtered_path1 = "D:/Accelerometer Data/Processed/"+experiment+"/"+week+"/"+day1+"/not_filtered/epoch_5/staudenmayer_2015/".replace('\\', '/')
non_filtered_path2 = "D:/Accelerometer Data/Processed/"+experiment+"/"+week+"/"+day2+"/not_filtered/staudenmayer_2015/".replace('\\', '/')
...
def evaluate_models(data, status):
    ...
    # Freedson intensity levels -> convert activity intensity to 3 levels
    data.loc[data['actilife_waist_intensity'] == 1, 'target_met_category_freedson_intensity'] = 1
    data.loc[data['actilife_waist_intensity'] == 2, 'target_met_category_freedson_intensity'] = 2
    data.loc[data['actilife_waist_intensity'] == 3, 'target_met_category_freedson_intensity'] = 3
    data.loc[data['actilife_waist_intensity'] == 4, 'target_met_category_freedson_intensity'] = 3

    # Freedson EE levels -> convert activity intensity to 3 levels
    data.loc[data['actilife_waist_ee'] < 3, 'target_met_category_freedson_ee'] = 1
    data.loc[(3 <= data['actilife_waist_ee']) & (data['actilife_waist_ee'] < 6), 'target_met_category_freedson_ee'] = 2
    data.loc[6 <= data['actilife_waist_ee'], 'target_met_category_freedson_ee'] = 3
    ...
    target_freedson_intensity = data['target_met_category_freedson_intensity'].fillna(1)
    target_freedson_ee = data['target_met_category_freedson_ee'].fillna(1)
    lr_estimated = data['lr_estimated_met_category'].fillna(1)
    ...
    """
    print("Linear Regression Equation with Intensity")

    # The mean squared error
    print("LR Mean squared error: %.2f" % np.mean((lr_estimated - target_freedson_intensity) ** 2))

    # The R squared score
    print("LR R squared score: %.2f" % r2_score(target_freedson_intensity, lr_estimated))

    # Precision and Recall for Linear Regression model
    precision, recall, fscore, support = precision_recall_fscore_support(target_freedson_intensity, lr_estimated, average='macro')
    ...
    precision, recall, fscore, support = precision_recall_fscore_support(target_freedson_intensity, lr_estimated)
    ...
    cnf_matrix = confusion_matrix(target_freedson_intensity, lr_estimated)
    np.set_printoptions(precision=2)

    # Plot non-normalized confusion matrix
    plt.figure(1)
    plot_confusion_matrix(cnf_matrix, classes=class_names, title=status + ' Linear Regression - Intensity')

    # print("\nLinear Regression Equation with Energy Expenditure")
    #
    # # The mean squared error
    # print("LR Mean squared error: %.2f" % np.mean((lr_estimated - target_freedson_ee) ** 2))
    #
    # # The R squared score
    # print("LR R squared score: %.2f" % r2_score(target_freedson_ee, lr_estimated))
    #
    # # Compute confusion matrix
    # cnf_matrix = confusion_matrix(target_freedson_ee, lr_estimated)
    # np.set_printoptions(precision=2)
    #
    # # Plot non-normalized confusion matrix
    # plt.figure(2)
    # plot_confusion_matrix(cnf_matrix, classes=class_names, title=status + ' Linear Regression - Energy Expenditure')

    print("\nDecision Tree with Intensity")

    # The mean squared error
    print("DT Mean squared error: %.2f" % np.mean((dt_estimated - target_freedson_intensity) ** 2))

    # The R squared score
    print("DT R squared score: %.2f" % r2_score(target_freedson_intensity, dt_estimated))

    # Precision and Recall for Decision Tree
    precision, recall, fscore, support = precision_recall_fscore_support(target_freedson_intensity, dt_estimated, average='macro')
    ...
    precision, recall, fscore, support = precision_recall_fscore_support(target_freedson_intensity, dt_estimated)
    ...
    # Compute confusion matrix
    cnf_matrix = confusion_matrix(target_freedson_intensity, dt_estimated)
    np.set_printoptions(precision=2)

    # Plot non-normalized confusion matrix
    plt.figure(3)
    plot_confusion_matrix(cnf_matrix, classes=class_names, title=status + ' Decision Tree - Intensity')


    # print("\nDecision Tree with Energy Expenditure")
    #
    # # The mean squared error
    # print("DT Mean squared error: %.2f" % np.mean((dt_estimated - target_freedson_ee) ** 2))
    #
    # # The R squared score
    # print("DT R squared score: %.2f" % r2_score(target_freedson_ee, dt_estimated))
    #
    # # Compute confusion matrix
    # cnf_matrix = confusion_matrix(target_freedson_ee, dt_estimated)
    # np.set_printoptions(precision=2)
    #
    # # Plot non-normalized confusion matrix
    # plt.figure(4)
    # plot_confusion_matrix(cnf_matrix, classes=class_names, title=status + ' Decision Tree - Energy Expenditure')

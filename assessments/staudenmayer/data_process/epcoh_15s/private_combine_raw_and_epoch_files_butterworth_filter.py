...
def process_with_filter(starting_row, end_row, experiment, week, day, user, date, device='Wrist'):
    ...
    epoch_filename = "D:\Accelerometer Data\ActilifeProcessedEpochs/Epoch15/"+experiment+"/"+week+"/"+day+"/processed/"+user+"_"+experiment+"_"+week.replace(' ', '_')+"_"+day+"_"+date+".csv".replace('\\', '/')
    path_components = wrist_raw_data_filename.split('/')

    output_path = "D:/Accelerometer Data/Processed"
    output_path = output_path + '/' + path_components[2] + '/' + path_components[3] + '/' + path_components[4] + '/filtered/epoch_15'
